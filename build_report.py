...
h3("4.1 本次產生的週期任務集（8 個任務，frame size = 4）")
table(
...
rich([("合計展開為 ", False), ("38 個週期 job", True), ("（> 30 ✔）；密度 D_W = Σ(e/p) = ", False),
      ("1.31 ≥ 0.7 ✔", True), ("；週期種類 7 種（≥3 ✔）；d=e 的有 p1、p3（2/8 = 25% ≥ 20% ✔）；"
       "非搶占且 e≠1 的有 p2、p5、p6、p7、p8（≥2 ✔）；frame size f=4 通過三條件（p8 最緊：2f−gcd(4,9)=7≤7 ✔）。"
       "本次實例所有任務皆為非搶占。", False)])
...
bullet("R10：有序對 (a,b)，在 b 的視窗內 e_a·x[b][t] ≤ Σ_{s<t} x[a][s]，即 a 全部做完前 b 不得啟動；"
       "重用 x，無新變數。未設定時程式自動挑一組可行的非平凡對（本次為 p2_0 → p4_0）。", bold_prefix="")
...
h3("10.1 動態排程的 9 輪再最佳化（共 9 次 MILP 解，約 12 秒，單執行緒、可重現）")
table(
...
para("9 輪皆無拒絕、無再排程失敗；38 個週期 job、6 個零星任務全部於硬截止前完成、10 個非週期任務全部於軟截止前完成。"
     "逐格驗證 C23 平衡 0 違反、SOC 維持在 [soc_min, soc_max] 0 違反、放寬儲能/機組/再生能源限制每輪皆成立。")

h3("10.2 靜態（L1）vs 動態（L2）效能比較")
table(
    ["指標", "靜態 L1", "動態 L2", "差異 Δ"],
    [["objective_value", "−69410.4", "−83238.8", "−13828.4（更佳）"],
     ["generator_cost", "296403.6", "311428.9", "+15025.3"],
     ["market_revenue", "385814.0", "394667.8", "+8853.8"],
...
bullet("靜態日前計畫雖一次看到所有任務，但只承諾一份排程，漏掉 2 個非週期任務（a2、a5，軟逾時率 0.2）。"
       "動態法隨任務到達再最佳化並為其保留容量，10 個全部準時完成（軟逾時率 0.0），平均回應時間由 3.63 降到 2.65。",
       bold_prefix="")
bullet("消除 2 個軟逾時 = 省下 2 × α = 20000 罰則，主導了 objective 的改善（−69410 → −83239）。")
bullet("代價清晰可見：保留容量並補償實際再生能源短缺使機組成本上升（+15025）；較大的承諾發電同時讓售電收益"
       "增加（+8854）。扣掉逾時罰則後，動態法在總目標上明顯更佳，印證 9.2 的權衡分析。")
...
script_block(
    ...
    "我們刻意先生成 d=e 的任務和非搶占任務來保證最難滿足的條件；並加了固定種子讓任務集可重現。"
    "這次產生 8 個週期任務，frame size 是 4，"
    "展開後是 38 個 job，超過 30 個；密度 1.31 遠高於 0.7 的下限；frame size 也通過 2f − gcd(f,p) ≤ d 這條。",
    ["報出關鍵數字：8 個任務、frame=4、38 個 job、密度 1.31。",
...
script_block(
    ...
    "這次動態排程跑了 9 輪、9 次 MILP，大約 12 秒（單執行緒、可重現）。任務隨時間陸續到達並被接受："
    "例如第 2 小時揭露 s1、a1、a2，第 28 小時揭露 s3、a5，一路到第 66 小時的 s6、a10。"
    "最後所有 38 個週期 job、6 個零星 job 都在硬截止前完成，10 個非週期 job 也都在軟截止前完成；"
    "硬逾時率 0、軟逾時率 0。每一格的供需平衡和 SOC 範圍我們也都逐格驗證沒有違反。",
    ["報出：9 輪、9 次解、約 12 秒、全部準時。",
...
script_block(
    ...
    "平均回應時間也從 3.63 降到 2.65。少漏 2 個就省下 2 萬元罰則，這主導了總目標從 −69410 改善到 −83239。"
    "代價是：保留容量加上補償太陽能實際短缺，機組成本多了約 1.5 萬；不過較大的發電也讓售電收益增加約 8.9 千。"
...
bullet("Q：動態法的計算成本？　A：本例 9 次解約 12 秒（單執行緒）；釘住前綴讓 presolve 消去已承諾段，"
       "再解規模隨時間縮小。")
bullet("Q：保證沒有新增決策變數？　A：所有放寬限制（R1–R10）與保留下限都只用 P/k/SOC/Sell/x，"
       "已用自我檢核逐條驗證。")
bullet("Q：completion_time_jitter 動態略增（44.1→45.8）為何仍較好？　A：jitter 是同任務各 instance "
